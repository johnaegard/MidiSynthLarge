...
const AudioSynthWavetable::instrument_data * const midi_map[] = {
  &glockenspiel, &epiano,&piano, &vibraphone, &harmonica,&nylonstrgtr, &steelstrgtr,&mutedgtr, &overdrivegt, 
  &distortiongt,&strings,&trumpet, &trombone, &tuba, &frenchhorn,
  &oboe, &bassoon, &clarinet,&flute, &recorder
};
...
const AudioSynthWavetable::instrument_data * channel_map[17] = {
	&strings, &piano, &overdrivegt, &distortiongt, &tuba, &frenchhorn, &piano, &piano, &piano, &piano, &standard_DRUMS, &piano, &piano, &piano, &piano, &piano, &piano,
};
